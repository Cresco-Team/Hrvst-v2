      ...
      <template v-for="slot in TIME_SLOTS" :key="slot.key">
        <div v-if="selectedSchedule[slot.key]">
          ...
          <div class="mb-4 flex flex-col gap-1.5 rounded-lg border bg-muted/20 p-3 pl-4 text-xs">
            <div class="flex items-center justify-between">
              <span class="flex items-center gap-1.5 text-muted-foreground">
                ...
                Supply
                <span class="text-muted-foreground/60">({{ selectedSchedule[slot.key]!.supply_posts_count }} posts)</span>
              </span>
            ...
            <div class="flex items-center justify-between">
              <span class="flex items-center gap-1.5 text-muted-foreground">
                ...
                Demand
                <span class="text-muted-foreground/60">({{ selectedSchedule[slot.key]!.demand_posts_count }} posts)</span>
              </span>
            ...
            <div class="flex items-center justify-between">
              ...
              <span
                ...
                :class="selectedSchedule[slot.key]!.net_kg > 0
          ...
          <div class="flex flex-col gap-2 pl-4">
            <div
              v-for="(item, idx) in selectedSchedule[slot.key]!.items"
              :key="idx"
            ...
            >
              <div class="flex min-w-0 items-center gap-2">
                <span
                  ...
                  :class="item.type === 'supply'
                    ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300'
                    : 'bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300'"
                >
                  {{ item.type }}
                </span>
                <span class="truncate text-xs font-medium">{{ item.variety_name }}</span>
              </div>
              <div class="ml-2 flex shrink-0 flex-col items-end gap-0.5">
                <span class="text-sm font-semibold tabular-nums">{{ formatKg(item.quantity_kg) }}</span>
                <span v-if="item.unit_price" class="text-[10px] text-muted-foreground">
                  ₱{{ item.unit_price.toFixed(2) }}/kg
                </span>
                <span v-else-if="item.price_flag" class="text-[10px] italic text-muted-foreground">
                  {{ item.price_flag }}
                </span>
              </div>
            </div>
