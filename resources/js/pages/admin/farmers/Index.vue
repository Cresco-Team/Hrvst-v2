...
watch(
    ...
    () => {
        if (currentView.value === 'map') fetchMarkers()
    },
    { immediate: true },
)

const storedView = localStorage.getItem('farmers_view') as 'list' | 'map' | null
if (storedView && storedView !== props.view) switchView(storedView)
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-6 p-4 lg:p-6">
            ...
            <!-- MAP VIEW — stacks on mobile, side-by-side on lg -->
            <div
                v-if="isMapView"
                class="flex flex-col gap-4 lg:grid lg:grid-cols-[1fr_320px]"
            >
                <div
                    class="relative min-h-[400px] w-full overflow-hidden rounded-lg border shadow-sm lg:min-h-[600px]"
                >
                    <Transition
