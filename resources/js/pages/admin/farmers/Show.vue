...
import type { BreadcrumbItem, DealerPostItemResource, FarmerResource } from '@/types'

const props = defineProps<{
	farmer?: FarmerResource
}>()

// supplyItems are PostItems — filter by PostItemStatus
                ...
                <div v-if="farmer" class="grid grid-cols-12 gap-5">
                        ...
                        <Card class="p-5 space-y-5">
                            <div class="flex flex-col items-start gap-3">
                                <Avatar class="size-16">
                                    <AvatarImage v-if="farmer.user?.avatar_url" :src="farmer.user.avatar_url" :alt="farmer.user.name" />
                            ...
                            <div class="space-y-2.5 text-sm text-muted-foreground">
                                <div class="flex items-center gap-2"><Mail class="size-4 shrink-0" /><span class="truncate">{{ farmer.user?.email }}</span></div>
                                <div class="flex items-center gap-2"><Phone class="size-4 shrink-0" /><span>{{ farmer.user?.phone_number }}</span></div>
                                <div class="flex items-center gap-2"><CalendarDays class="size-4 shrink-0" /><span>Joined {{ farmer.joined_at_human }}</span></div>
                            </div>
                        </Card>
                    </div>
                    ...
                    <div class="col-span-12 lg:col-span-9 space-y-4">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            ...
                            <SmallCard title="Ongoing Items" :value="ongoingItems.length" />
                        </div>
                            ...
                            <CardContent class="pt-4">
                                <Tabs default-value="ongoing">
                                    <TabsList class="mb-4">
                                        ...
                                        <TabsTrigger value="growing" class="gap-1.5">
                                            <Sprout class="size-4" />Growing
                                            <Badge variant="secondary" class="ml-1 px-1.5 py-0 text-xs">{{ growingCount }}</Badge>
                                        </TabsTrigger>
                                    </TabsList>
                                    ...
                                    <TabsContent value="growing">
                                        <div class="flex items-center justify-center h-24 text-sm text-muted-foreground">
                                            {{ growingCount }} crop(s) registered, not yet harvested.
                                        </div>
                                    </TabsContent>
                                </Tabs>
                            </CardContent>
