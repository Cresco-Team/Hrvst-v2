...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex flex-col gap-6 p-4 lg:p-6">
      ...
      <div v-if="categories.length === 0" class="flex items-center justify-center rounded-lg border border-dashed py-16 text-sm text-muted-foreground">
        No categories found.
      </div>
          ...
          <Card class="h-full transition-all hover:shadow-md hover:-translate-y-0.5 cursor-pointer">
            <CardHeader class="pb-3">
              <div class="flex items-start justify-between gap-3">
                ...
                <ArrowRight
                  class="size-4 shrink-0 text-muted-foreground transition-transform group-hover:translate-x-0.5 group-hover:text-foreground mt-0.5"
                />
              </div>
            </CardHeader>
            ...
            <CardContent class="pt-0">
              <div class="flex items-center gap-3">
                <div class="flex items-center gap-1.5 text-sm text-muted-foreground">
                  ...
                  <span class="font-medium tabular-nums text-foreground">{{ category.vegetables_count }}</span>
                  <span>{{ category.vegetables_count === 1 ? 'vegetable' : 'vegetables' }}</span>
                </div>
                ...
                <div class="flex items-center gap-1.5 text-sm text-muted-foreground">
                  ...
                  <span class="font-medium tabular-nums text-foreground">{{ category.varieties_count }}</span>
                  <span>{{ category.varieties_count === 1 ? 'variety' : 'varieties' }}</span>
                </div>
              </div>
            </CardContent>
          </Card>
