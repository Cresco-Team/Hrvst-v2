...
<template>
    <div class="min-h-screen antialiased"
        ...
        <nav class="fixed top-0 z-50 w-full border-b bg-background/75 border-border backdrop-blur-md">
            <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
                <Link href="/" class="flex items-center gap-2.5 select-none">
                    ...
                    <span class="text-lg font-semibold tracking-tight" style="color: var(--foreground);">Hrvst</span>
                </Link>

                <div class="flex items-center gap-3">
                    <template v-if="page.props.auth.user">
                        <Link :href="logout()" class="text-sm font-medium transition-opacity hover:opacity-70"
        ...
        <section
            ...
            <div class="relative z-10 max-w-5xl">
                ...
                <h1 class="hero-title mb-6 text-5xl font-black leading-[1.05] tracking-tight sm:text-7xl lg:text-[5.5rem]"
                    ...
                    Where Farmers<br>
                    <span style="color: var(--primary);">Meet Dealers.</span>
                </h1>
            ...
            <div class="mx-auto max-w-7xl">
                ...
                <div class="grid gap-6 lg:grid-cols-2">
                    ...
                    <div class="relative overflow-hidden rounded-2xl p-8 lg:p-12"
                        style="background: oklch(0.7920 0.2090 151.7110 / 0.06); border: 1px solid oklch(0.7920 0.2090 151.7110 / 0.2);">
                        <div class="pointer-events-none absolute -right-16 -top-16 size-64 rounded-full opacity-20"
                            aria-hidden="true"
                            style="background: radial-gradient(circle, var(--primary), transparent);" />

                        <div class="relative z-10">
                            <div class="mb-6 inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-widest"
                                style="background: oklch(0.7920 0.2090 151.7110 / 0.12); color: oklch(0.5000 0.2090 151.7110);">
                            ...
                            <ul class="space-y-4">
                                <li v-for="item in farmerBenefits" :key="item.title"
                                    class="flex items-start gap-3 text-sm" style="color: var(--muted-foreground);">
                                    <component :is="item.icon" class="mt-0.5 size-4 shrink-0"
                                        style="color: var(--primary);" />
                                    <span>
                                        <strong class="font-semibold" style="color: var(--foreground);">{{ item.title
                                            }}.
                                        </strong>{{ item.desc }}
                                    </span>
                                </li>
                    ...
                    <!-- Dealer card -->
                    <div class="relative overflow-hidden rounded-2xl p-8 lg:p-12"
                        style="background: oklch(0.7920 0.2090 151.7110 / 0.03); border: 1px solid oklch(0.7920 0.2090 151.7110 / 0.14);">
                        <div class="pointer-events-none absolute -left-16 -bottom-16 size-64 rounded-full opacity-15"
                            aria-hidden="true"
                            style="background: radial-gradient(circle, var(--primary), transparent);" />

                        <div class="relative z-10">
                            <div class="mb-6 inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-widest"
                                style="background: oklch(0.7920 0.2090 151.7110 / 0.10); color: oklch(0.5000 0.2090 151.7110);">
                            ...
                            <ul class="space-y-4">
                                <li v-for="item in dealerBenefits" :key="item.title"
                                    class="flex items-start gap-3 text-sm" style="color: var(--muted-foreground);">
                                    <component :is="item.icon" class="mt-0.5 size-4 shrink-0"
                                        style="color: var(--primary);" />
                                    <span>
        ...
        <section class="px-6 py-24" style="background: var(--background);">
            <div class="mx-auto max-w-3xl">
                ...
                <div class="relative space-y-0">
                    <div class="absolute left-6 top-6 hidden h-[calc(100%-3rem)] w-px lg:block" aria-hidden="true"
                        style="background: linear-gradient(to bottom, var(--primary), transparent);" />

                    <div v-for="(step, index) in steps" :key="step.step" class="flex gap-8 lg:gap-12"
                        :class="index < steps.length - 1 ? 'pb-12' : ''">
                        <div class="relative flex shrink-0 flex-col items-center">
                            <div class="flex size-12 items-center justify-center rounded-full text-xs font-bold tracking-wider shadow-md"
                                style="background: var(--primary); color: var(--primary-foreground);">
                                {{ step.step }}
                            </div>
                        </div>
                        ...
                        <div class="pb-1 pt-2">
                            <h3 class="mb-1.5 text-xl font-bold" style="color: var(--foreground);">{{ step.title }}</h3>
                            <p class="text-sm leading-relaxed" style="color: var(--muted-foreground);">{{ step.desc }}
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <!-- ═══════════════════════════════════════ FOOTER ═══ -->
        <footer class="px-6 py-8" style="background: var(--card); border-top: 1px solid var(--border);">
            <div class="mx-auto flex max-w-7xl items-center justify-between">
                <div class="flex items-center gap-2">
                    <AppLogoIcon class="size-5 fill-current opacity-40" style="color: var(--muted-foreground);" />
                    <span class="text-sm font-semibold opacity-50" style="color: var(--muted-foreground);">Hrvst</span>
                </div>
                <p class="text-xs opacity-50" style="color: var(--muted-foreground);">Trading Post ·
                    Team Cresco</p>
            </div>
        </footer>
    </div>
