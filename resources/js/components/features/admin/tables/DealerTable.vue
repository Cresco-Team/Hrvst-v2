...
<template>
    <DataTable :data="dealers" :columns="columns" :search-query="searchQuery" search-placeholder="Search dealers..."
        ...
        <template #cell-expander="{ row, cell }">
            <Button v-if="(row.ongoing_demands_count ?? 0) > 0" variant="ghost" size="icon-sm"
        ...
        <template #cell-ongoing_demands_count="{ row }">
            <div class="flex items-center gap-2">
                ...
                <span class="font-mono font-medium">{{ row.ongoing_demands_count ?? 0 }}</span>
            </div>
        </template>
            ...
            <TooltipProvider v-if="row.joined_at" :delay-duration="200">
                <Tooltip>
                    <TooltipTrigger as-child>
                        <div class="cursor-help text-sm">{{ row.joined_at_human }}</div>
                    </TooltipTrigger>
                    <TooltipContent>
                ...
                <td :colspan="colspan" class="px-4 py-4">
                    <div class="ml-12">
                        <h4 class="mb-3 text-sm font-medium">
                            Ongoing Demands ({{ row.demands?.length ?? 0 }})
                        </h4>
