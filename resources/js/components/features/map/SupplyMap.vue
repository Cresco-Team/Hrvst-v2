...
import type { MapConfig, SupplyMarker } from '@/types'
...
const emit = defineEmits<{
	'marker-click': [marker: SupplyMarker]
}>()

const { container, init, getMap } = useLeafletMap({
	zoom: props.config.zoom ?? 10,
	placeFallbackMarker: false,
})
...
function getDominantColor(breakdown: SupplyMarker['supply_breakdown']): string {
...
function buildMarkerIcon(leaflet: typeof L, marker: SupplyMarker): L.DivIcon {
...
async function renderMarkers(): Promise<void> {
	...
	if (clusterGroup) {
		...
		clusterGroup = (L as unknown as { markerClusterGroup: (opts: unknown) => MarkerClusterGroup })
			.markerClusterGroup({
				maxClusterRadius: 80,
