    ...
    <Card class="group relative overflow-hidden border-2 transition-all hover:shadow-lg hover:border-primary/20">
        ...
        <CardContent class="p-0">
            ...
            <div class="relative h-48 overflow-hidden bg-gradient-to-br from-primary/5 to-primary/10">
                <img
                    v-if="planting.image_url"
                    :src="planting.image_url"
            ...
            <div class="p-5 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div class="flex items-center gap-3">
                        <div class="flex size-10 shrink-0 items-center justify-center rounded-lg bg-blue-100 dark:bg-blue-950/30">
                            <Weight class="size-5 text-blue-600 dark:text-blue-400" />
                        </div>
                        <div class="flex-1">
                            <p class="text-xs text-muted-foreground">Weight</p>
                            <p class="text-sm font-bold">{{ planting.weight_kg }} kg</p>
                        </div>
                    </div>

                    <div class="flex items-center gap-3">
                        <div class="flex size-10 shrink-0 items-center justify-center rounded-lg bg-green-100 dark:bg-green-950/30">
                            <DollarSign class="size-5 text-green-600 dark:text-green-400" />
                        </div>
                        <div class="flex-1">
                            <p class="text-xs text-muted-foreground">Price</p>
                            <p class="text-sm font-bold">₱{{ planting.asking_price }}/kg</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="rounded-lg border bg-muted/30 p-3">
                    ...
                    <p 
                        v-if="planting.days_until_expiration !== null" 
                        class="text-xs font-medium mt-1"
                        :class="getExpirationColor(planting.days_until_expiration)"
                    >
                        <span v-if="planting.days_until_expiration > 0">
                            {{ planting.days_until_expiration }} days left
                        </span>
                        <span v-else-if="planting.days_until_expiration === 0">
                            Today!
                        </span>
                        <span v-else>
                            {{ Math.abs(planting.days_until_expiration) }} days overdue
                        </span>
                    </p>
                </div>
                ...
                <div class="text-xs text-muted-foreground">
                    Posted {{ planting.created_at_human }}
                </div>
