...
<template>
    <Card class="py-0 gap-2 overflow-hidden transition-all hover:shadow-lg ">
        <AspectRatio :ratio="16 / 9" class="relative overflow-hidden">
            <img v-if="supply.image_url" :src="supply.image_url" :alt="supply.variety.name.charAt(0)"
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                    <DropdownMenuItem v-if="isOngoing" @click="emit('edit', supply)">
                    ...
                    <DropdownMenuItem v-if="isOngoing || isFulfilled" @click="emit('archive', supply)"
                        class="text-orange-600 dark:text-orange-400">
                        <Archive class="mr-2 size-4" />
                        Archive
                    </DropdownMenuItem>

                    <DropdownMenuItem v-if="isOngoing || isArchived" @click="emit('fulfill', supply)"
                        class="text-green-500 dark:text-green-400">
                        <PackageCheck class="mr-2 size-4" />
                        Fulfill
                    </DropdownMenuItem>

                    <DropdownMenuItem @click="emit('delete', supply)" class="text-destructive">
