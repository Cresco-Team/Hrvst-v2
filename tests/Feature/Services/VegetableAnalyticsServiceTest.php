...
it('confirms the fixture actually produces an undersupply band with a supply decline', function () {
    ...
    $dto = (new VegetableAnalyticsService)->compute(
...
it('gives each role role-specific undersupply copy, not a shared generic sentence', function () {
    $admin = (new VegetableAnalyticsService)->compute(undersupplyMonthlyActivity(), VegetableViewerRole::Admin);
    $farmer = (new VegetableAnalyticsService)->compute(undersupplyMonthlyActivity(), VegetableViewerRole::Farmer);
    $dealer = (new VegetableAnalyticsService)->compute(undersupplyMonthlyActivity(), VegetableViewerRole::Dealer);
